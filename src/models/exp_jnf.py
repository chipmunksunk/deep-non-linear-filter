...
from models.models import FTJNF, JNF_PF
...
class JNFExp_PF(EnhancementExp):
    ...
    def __init__(self,
                 ...
                 cirm_comp_C: float, 
                 reference_channel: int = 0):
        super(JNFExp_PF, self).__init__(model=model, cirm_comp_K=cirm_comp_K, cirm_comp_C=cirm_comp_C)
    ...
    def evaluate_single_utterance(self, dm, idx):
        ...
        # Hashir: If input is singlechannel, stack over frequency dimensions which serves as the feature dimension in the post-filter.
        stacked_noisy_stft = torch.concat((torch.real(noisy_stft), torch.imag(noisy_stft)), dim=2)
        ...
        # compute estimates
        est_clean_stft = noisy_stft * speech_mask
        est_noise_stft = noisy_stft * noise_mask
        clean_td, noise_td, est_clean_td, est_noise_td = self.get_td_rep(clean_stft, noise_stft,
                                                                         est_clean_stft, est_noise_stft)
    ...
    def shared_step(self, batch, batch_idx, stage: Literal['train', 'val']):
        ...
        # compute estimates
        speech_mask = speech_mask[:, self.reference_channel, ...]
        noise_mask = noise_mask[:, self.reference_channel, ...]
        est_clean_stft = noisy_stft[:, self.reference_channel, ...] * speech_mask
        est_noise_stft = noisy_stft[:, self.reference_channel, ...] * noise_mask
        clean_td, noise_td, est_clean_td, est_noise_td = self.get_td_rep(clean_stft[:, self.reference_channel, ...], noise_stft[:, self.reference_channel, ...],
                                                                         est_clean_stft, est_noise_stft)
