...
#Hashir: JNF for post-filtering purposes. The architecture is the same as JNF but with the frequency and time dimensions switched to act as a tempo-spectral post-filter. The input is expected to be single-channel and the output mask is applied to the single-channel input STFT.    
class JNF_PF(nn.Module):
    ...
    def __init__(self,
        ...
        lstm_input = 2*n_freqs # because of real and imaginary part
        ...
        self.linear_out_features = self.linear_out_features * n_freqs
        self.ff = nn.Linear(2*self.n_lstm_hidden2, out_features=self.linear_out_features)
